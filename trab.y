...
int insertSymbTab (char *symb, Entity whichEntt) {
  ...
  if (existingSym < topTab && whichEntt == Constant) return existingSym;
  ...
  current = topTab;
  if (whichEntt == Constant || (whichEntt == Constant)) {
...
D    : D V _PTVIRG 
     | V _PTVIRG
V    : V _VIRG _V {insertSymbTab($1.symbol, Variable);
     | _INT _V {insertSymbTab($1.symbol, Variable);
...
T    : T _MULT F {	
      ...
      | F	 {	
                 $$.intval = $1.intval;}
F    : _V {$$.intval=returnSymbTab($1.symbol, Variable);
          }

     | _N {$$.intval=returnSymbTab($1.symbol, Constant);
          } 
     ;
%%
